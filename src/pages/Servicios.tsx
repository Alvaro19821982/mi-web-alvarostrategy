...
const serviceColorClasses = {
  blue: {
    ...
    badgeBg: "bg-blue-500 hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-700", border: "border-blue-500 dark:border-blue-600",
  ...
  indigo: {
    iconBg: "bg-indigo-100 dark:bg-indigo-900/30", iconText: "text-indigo-600 dark:text-indigo-400",
    badgeBg: "bg-indigo-500 hover:bg-indigo-600 dark:bg-indigo-600 dark:hover:bg-indigo-700", border: "border-indigo-500 dark:border-indigo-600",
  ...
  purple: {
    ...
    badgeBg: "bg-purple-500 hover:bg-purple-600 dark:bg-purple-600 dark:hover:bg-purple-700", border: "border-purple-500 dark:border-purple-600",
...
const ServiceCard: React.FC<{ service: ServiceItem, getPath: (key: string) => string }> = ({ service, getPath }) => {
  ...
  const descriptionHasLink = service.descriptionKey.includes('<1>');
  const finalPath = service.pathKey === 'homepage' ? `/${currentLang}` : getPath(service.pathKey);
      ...
      <Card className={cn(
        ...
        <CardHeader className="p-6 sm:p-8">
          <div className="flex items-center mb-4">
            <div className={cn("p-3 rounded-full mr-4 shadow-md group-hover:scale-110 transition-transform", colors.iconBg)}>
              {React.cloneElement(service.icon, { className: cn(service.icon.props.className, colors.iconText) })}
            </div>
            {service.badgeKey && (
        ...
        <CardContent className="p-6 sm:p-8 pt-0 flex-grow flex flex-col">
          <CardDescription className="text-sm sm:text-base text-gray-600 dark:text-slate-300 leading-relaxed mb-6 flex-grow">
            {descriptionHasLink ? (
              <Trans i18nKey={service.descriptionKey}>
                ...<Link to={getPath('routes.method')} className="font-semibold text-blue-600 dark:text-blue-400 hover:underline">...</Link>...
              </Trans>
            ) : (
...
const ServiciosPage = () => {
  ...
  const servicesData: ServiceItem[] = [
    { icon: <Lightbulb className="w-7 h-7 sm:w-8 sm:h-8" />, titleKey: "servicesPage.service1_title", descriptionKey: "servicesPage.service1_desc", pathKey: "routes.service_edi", linkKey: "servicesPage.service1_link", colorClass: "blue" },
    { icon: <Search className="w-7 h-7 sm:w-8 sm:h-8" />, titleKey: "servicesPage.service2_title", descriptionKey: "servicesPage.service2_desc", pathKey: "routes.service_seo", linkKey: "servicesPage.service2_link", colorClass: "indigo" },
    { icon: <Brain className="w-7 h-7 sm:w-8 sm:h-8" />, titleKey: "servicesPage.service3_title", descriptionKey: "servicesPage.service3_desc", pathKey: "routes.service_ia", linkKey: "servicesPage.service3_link", colorClass: "purple" },
    { icon: <BookUser className="w-7 h-7 sm:w-8 sm:h-8" />, titleKey: "servicesPage.service4_title", descriptionKey: "servicesPage.service4_desc", pathKey: "homepage", linkKey: "servicesPage.service4_link", badgeKey: "servicesPage.service4_badge", colorClass: "blue" }
  ];

  const preFooterLinks = [
    { labelKey: "servicesPage.prefooter_link1_label", pathKey: "routes.method", icon: <Cog className="w-8 h-8 mx-auto mb-3 text-blue-600 dark:text-blue-400" />, descriptionKey: "servicesPage.prefooter_link1_desc" },
    { labelKey: "servicesPage.prefooter_link2_label", pathKey: "routes.whoAmI", icon: <QuienSoyIcon className="w-8 h-8 mx-auto mb-3 text-indigo-600 dark:text-indigo-400" />, descriptionKey: "servicesPage.prefooter_link2_desc" },
    { labelKey: "servicesPage.prefooter_link3_label", pathKey: "routes.blog", icon: <BlogIconFile className="w-8 h-8 mx-auto mb-3 text-purple-600 dark:text-purple-400" />, descriptionKey: "servicesPage.prefooter_link3_desc" },
  ];

  const schemaMarkup = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    "name": t('servicesPage.meta_title'),
    "description": t('servicesPage.meta_description'),
    ...
    "breadcrumb": {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 2, "name": t('servicesPage.breadcrumb') }
      ]
    },
    "mainEntity": {
      "@type": "OfferCatalog",
      "name": `Catálogo de servicios de AlvaroStrategy (${currentLang})`,
      "itemListElement": servicesData
        .filter(service => service.pathKey !== "homepage") 
        .map(service => ({
          "@type": "Offer",
          "itemOffered": {
            "@type": "Service",
            "name": t(service.titleKey),
            "url": `https://alvarostrategy.com${getPath(service.pathKey)}`, 
            "description": t(service.descriptionKey).replace(/<1>.*<\/1>/, '')
          }
      }))
    }
  };
  ...
  return (
    <>
      <SeoTags
        title={t('servicesPage.meta_title')}
        description={t('servicesPage.meta_description')}
      ...
      <div className="bg-slate-50 dark:bg-slate-900 text-gray-800 dark:text-slate-200 py-12 sm:py-16 md:py-20">
        <motion.div
          initial="hidden"
          animate="visible"
        ...
        >
          <motion.div variants={fadeInUp} className="mb-6 sm:mb-8">
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbSeparator />
                <BreadcrumbItem>
                  <BreadcrumbPage className="font-medium text-gray-700 dark:text-slate-200">{t('servicesPage.breadcrumb')}</BreadcrumbPage>
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          </motion.div>
          ...
          <motion.header variants={fadeInUp} className="text-center mb-12 md:mb-16">
            <Badge className="mb-5 sm:mb-6 bg-gradient-to-r from-blue-600 to-indigo-700 text-white px-5 py-2.5 sm:px-6 sm:py-3 text-sm sm:text-base font-semibold rounded-lg shadow-xl cursor-default">
              <Briefcase className="w-5 h-5 mr-2 inline-block" /> {t('servicesPage.badge')}
            </Badge>
          ...
          <motion.section variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} className="mb-16 md:mb-20">
            <h2 className="text-3xl sm:text-4xl font-bold text-gray-800 dark:text-slate-100 text-center mb-10 md:mb-12">
              <Trans i18nKey="servicesPage.expertise_title">
                Nuestras áreas de <span className="text-blue-600 dark:text-blue-400">expertise</span>
              </Trans>
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8 lg:gap-10">
              {servicesData.map((service) => (
                <ServiceCard key={service.titleKey} service={service} getPath={getPath} />
              ))}
            </div>
          </motion.section>

          <motion.section variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} className="my-16 md:my-20 lg:my-24">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-slate-100 text-center mb-10 md:mb-12">
              {t('servicesPage.prefooter_title')}
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 md:gap-8">
              {preFooterLinks.map(item => (
                <motion.div variants={fadeInUp} key={item.pathKey}>
          ...
          <motion.section variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} className="bg-gradient-to-br from-blue-600 via-indigo-700 to-purple-800 text-white p-8 sm:p-10 lg:p-14 rounded-2xl shadow-2xl text-center">
            <HelpCircle className="w-12 h-12 sm:w-14 sm:h-14 mx-auto mb-5 text-sky-300" />
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4">{t('servicesPage.cta_title')}</h2>
            <p className="text-lg sm:text-xl text-blue-100/90 mb-6 max-w-2xl mx-auto leading-relaxed">{t('servicesPage.cta_desc')}</p>
            <Button
              size="lg"
              ...
              onClick={() => navigate(getPath('routes.contact'), { state: { subject: "Solicitud de sesión estratégica gratuita desde página de servicios" } })}
            >
              <MessageSquareHeart className="mr-2 w-5 h-5 sm:w-5 sm:h-5" />
              {t('servicesPage.cta_button')}
              <ArrowRight className="ml-2 w-5 h-5 group-hover:translate-x-1.5 transition-transform" />
            </Button>
            <p className="text-sm text-blue-200/80 mt-5 italic">{t('servicesPage.cta_note')}</p>
          </motion.section>
        </motion.div>
      </div>
    </>
