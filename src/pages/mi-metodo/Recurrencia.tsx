...
const Recurrencia = () => {
  ...
  const recurrenceStrategies: RecurrenceStrategy[] = [
    { titleKey: "method_recurrence.strategy1_title", icon: <Mail className="w-7 h-7"/>, descriptionKey: "method_recurrence.strategy1_desc", detailKeys: ["method_recurrence.strategy1_d1", "method_recurrence.strategy1_d2", "method_recurrence.strategy1_d3", "method_recurrence.strategy1_d4"], colorClass: "text-red-600" },
    { titleKey: "method_recurrence.strategy2_title", icon: <Gift className="w-7 h-7"/>, descriptionKey: "method_recurrence.strategy2_desc", detailKeys: ["method_recurrence.strategy2_d1", "method_recurrence.strategy2_d2", "method_recurrence.strategy2_d3"], colorClass: "text-rose-500" },
    { titleKey: "method_recurrence.strategy3_title", icon: <Heart className="w-7 h-7"/>, descriptionKey: "method_recurrence.strategy3_desc", detailKeys: ["method_recurrence.strategy3_d1", "method_recurrence.strategy3_d2", "method_recurrence.strategy3_d3"], colorClass: "text-pink-600" },
    { titleKey: "method_recurrence.strategy4_title", icon: <Users className="w-7 h-7"/>, descriptionKey: "method_recurrence.strategy4_desc", colorClass: "text-orange-500" },
    { titleKey: "method_recurrence.strategy5_title", icon: <Award className="w-7 h-7"/>, descriptionKey: "method_recurrence.strategy5_desc", colorClass: "text-amber-500" },
    { titleKey: "method_recurrence.strategy6_title", icon: <Repeat className="w-7 h-7"/>, descriptionKey: "method_recurrence.strategy6_desc", colorClass: "text-red-700" },
  ];
  
  const pageTitle = t('method_recurrence.meta_title');
  const pageDescription = t('method_recurrence.meta_description');
  ...
  const schemaMarkup = {
    ...
    "breadcrumb": {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 3, "name": t('method_recurrence.breadcrumb') }
      ]
    },
  };
        ...
        >
          <div className="max-w-5xl mx-auto px-4 sm:px-6 text-center">
            <motion.div variants={fadeInUp} className="mb-5">
                <Link to={getPath('routes.method')} className="inline-flex items-center text-red-200 hover:text-white transition-colors group text-sm">
                    <ArrowLeft className="w-4 h-4 mr-2 group-hover:-translate-x-0.5 transition-transform" />
                    {t('method_recurrence.back_to_method')}
                </Link>
            </motion.div>
            <motion.div variants={fadeInUp}>
              <Badge className="mb-5 sm:mb-6 bg-white/10 dark:bg-white/5 border border-white/20 text-white px-4 py-1.5 backdrop-blur-sm text-xs sm:text-sm font-medium rounded-full shadow-sm">
                {t('method_recurrence.phase_badge')}
              </Badge>
            </motion.div>
            <motion.h1
              variants={fadeInUp}
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6">
                ...
                <BreadcrumbList>
                  <BreadcrumbItem>
                    <BreadcrumbLink asChild>
                      <Link to={`/${currentLang}`} className="text-sm flex items-center text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors">
                  ...
                  <BreadcrumbItem>
                    <BreadcrumbLink asChild>
                      <Link to={getPath('routes.method')} className="text-sm text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors">
                  ...
                  <BreadcrumbSeparator />
                  <BreadcrumbItem>
                    <BreadcrumbPage className="text-sm font-medium text-gray-700 dark:text-slate-200">{t('method_recurrence.breadcrumb')}</BreadcrumbPage>
                  </BreadcrumbItem>
                </BreadcrumbList>
              </Breadcrumb>
            </motion.div>
            ...
            >
              <h2 className="text-2xl sm:text-3xl flex items-center mb-6 sm:mb-8 dark:text-slate-100">
                ...
                {t('method_recurrence.section1_title')}
              </h2>
              <p className="mb-5 sm:mb-6 leading-relaxed sm:leading-loose">
                <Trans i18nKey="method_recurrence.section1_p1">
                    ...<strong className="text-red-600 dark:text-red-400">...</strong>...<strong className="text-red-600 dark:text-red-400">...</strong>...
                </Trans>
              </p>
              <p className="mb-5 sm:mb-6 leading-relaxed sm:leading-loose">
                {t('method_recurrence.section1_p2')}
              </p>
              <blockquote className="border-l-4 pl-4 sm:pl-6 italic my-6 sm:my-8 py-2 sm:py-3">
                {t('method_recurrence.section1_blockquote')}
              </blockquote>
               <p className="mb-5 sm:mb-6 leading-relaxed sm:leading-loose">
                {t('method_recurrence.section1_p3')}
              </p>
            </motion.section>
            ...
            >
              <motion.h2 variants={fadeInUp} className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 dark:text-slate-100 text-center mb-10 md:mb-12 lg:mb-16">
                 <Trans i18nKey="method_recurrence.section2_title">Estrategias clave para fomentar la <span className="text-red-600 dark:text-red-400">recurrencia y lealtad</span></Trans>
              </motion.h2>
                  ...
                  <motion.div key={strategy.titleKey} variants={fadeInUp} className="h-full flex">
                    <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300 border border-gray-200/70 dark:border-slate-700/50 rounded-xl overflow-hidden w-full bg-white dark:bg-slate-800/70 flex flex-col">
                      ...
                      <CardContent className="p-5 sm:p-6 flex-grow flex flex-col">
                        ...
                        {strategy.detailKeys && strategy.detailKeys.length > 0 && (
                            <div className="mt-auto pt-3 border-t border-gray-200/30 dark:border-slate-700/50">
                                <CardDescription className="text-xs text-gray-500 dark:text-slate-400 mb-1.5">{t('method_recurrence.details_label')}</CardDescription>
                                <ul className="list-disc list-inside text-xs sm:text-sm text-gray-600 dark:text-slate-400 space-y-1">
                                    {strategy.detailKeys.map(detailKey => <li key={detailKey}>{t(detailKey)}</li>)}
                                </ul>
                            </div>
                        )}
                      </CardContent>
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
                <Button size="lg" variant="outline" asChild className="group border-slate-400 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 w-full sm:w-auto transition-all duration-300">
                    <Link to={getPath('routes.method_progression')} aria-label={t('method_recurrence.cta_button_prev')}>
                        <ArrowLeft className="mr-2 w-5 h-5 group-hover:-translate-x-1 transition-transform" />
                        {t('method_recurrence.cta_button_prev')}
                    </Link>
                </Button>
                <Button size="lg" asChild className="group bg-red-600 hover:bg-red-700 dark:bg-red-500 dark:hover:bg-red-600 text-white w-full sm:w-auto transition-all duration-300 transform hover:scale-105">
                    <Link to={getPath('routes.method_scalability')} aria-label={t('method_recurrence.cta_button_next')}>
                    {t('method_recurrence.cta_button_next')}
              ...
              <div className="mt-8">
                 <Button size="sm" variant="ghost" asChild className="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300">
                    <Link to={getPath('routes.contact')} state={{ subject: t('method_recurrence.cta_button_contact') }}>
                        <Lightbulb className="w-4 h-4 mr-2" /> {t('method_recurrence.cta_button_contact')}
                    </Link>
                </Button>
               </div>
            </motion.section>
